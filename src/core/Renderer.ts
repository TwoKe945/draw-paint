...
export class CanvasRenderer  extends Renderer {
  ...
  protected handleClickBlank() {
    ...
    const rectangle = this.find((component) => {
    ...
    this.isClickBlank = !rectangle
  }

  protected handleComponetsOnMouseDown(_e: MouseEvent){
    this.selectedComponent?.onMouseDown(this.mousePosition, _e)
  }
  ...
  protected handleComponetOnMouseDown(_e: MouseEvent) {
    const rectangle = this.find((component) => component.isInside(this.mousePosition.x, this.mousePosition.y)) as RenderComponent;
    if (!rectangle) return
    this.clearSelectedComponent();
    if (!rectangle.isMouseDown) {
      rectangle.isMouseDown = true;
      rectangle.isSelected = true;
      this.selectedComponent = rectangle;
      this.mouseOffsetX = this.mousePosition.x - rectangle.x;
      this.mouseOffsetY = this.mousePosition.y - rectangle.y;
      this.mouseOffsetWidth = rectangle.width;
      this.mouseOffsetHeight = rectangle.height;
      this.canvas!.style.cursor = rectangle.defaultCursor;
    }
  }
  ...
  protected handleComponetsOnMouseUp(_e: MouseEvent) {
    this.selectedComponent?.onMouseUp(this.mousePosition, _e)
  }
  ...
  protected handleComponetsOnMouseMove(e: MouseEvent) {
    this.selectedComponent?.onMouseMove(this.mousePosition, e)
  }
